...
2. **不许改 `jzt_common.js`**——它是官方公共库，各站点应保持一致（校验：与本 skill `assets/js/jzt_common.js` 或 demo 仓库内容一致）。
...
公共库已内置在本 skill `assets/` 下（`js/jquery.min.js`、`js/vue.global.prod.js`、`js/jzt_common.js`、`js/swiper-bundle.min.js`、`css/swiper-bundle.min.css`），新项目直接复制到项目的 `js/`、`css/` 目录，无需联网下载。
